...
import iris
from iris import createConnection
...
connection = createConnection("iris", 1972, "USER", "SuperUser", "SYS", sharedmemory = False)

def director_create_business_service(connection, serviceName):
    irisInstance = iris.IRIS(connection)

    return irisInstance.classMethodObject("EnsLib.PEX.Director","dispatchCreateBusinessService",serviceName)
...
def get_all_persons():
    ...
    service = director_create_business_service(connection,"Python.FlaskService")
    response = service.invokeObject('OnProcessInput',msg)
...
def post_person():
    ...
    service = Director.CreateBusinessService(connection,"Python.FlaskService")
